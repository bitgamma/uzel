...
exports.ApplianceManager.prototype.start = function() {
  ...
  this._io.of('/devices').on('connection', handleDevicesSocketConnection.bind(this));
...
function handleDevicesSocketConnection(socket) {
  ...
  socket.on('discoverDevices', function() {
    this.deviceManager.discoverDevices(handleDeviceDiscovered.bind(this, socket));
  }.bind(this));
  
  socket.on('pairDevice', function(deviceDescriptor) {
    var device = this.deviceManager.getUnpairedDeviceByFullID(deviceDescriptor.id);
  ...
  socket.on('unpairDevice', function(deviceDescriptor) {
    var device = this.deviceManager.getDeviceByFullID(deviceDescriptor.id);
    this.deviceManager.unpair(device, handleDeviceUnpaired.bind(this, socket));
  }.bind(this));
}

function handleDeviceDiscovered(socket, device) {
  var desc = deviceDescriptor(device);
  socket.emit('deviceDiscovered', desc);
  socket.broadcast.emit('deviceDiscovered', desc);
}

function handleDevicePaired(socket, device, err) {  
  if(!err) {
    var desc = deviceDescriptor(device);
    socket.emit('devicePaired', desc);    
    socket.broadcast.emit('devicePaired', desc);    
  }
}
...
function handleDeviceUnpaired(socket, device, err) {
  if(!err) {
    var desc = deviceDescriptor(device);
    socket.emit('deviceUnpaired', desc);    
    socket.broadcast.emit('deviceUnpaired', desc);    
  }
}
