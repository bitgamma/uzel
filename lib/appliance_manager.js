...
function index(req, res) {
  ...
  for (var moduleName in this._applianceLoader.loadedModules) {
    moduleDescriptors.push({ url: '/' + moduleName, icon: '/' + moduleName + '/logo.svg', label: idToLabel(moduleName)});
  }

  moduleDescriptors.push({ url: '/device_manager', icon: '/img/device_manager.svg', label: 'Device Manager'});
