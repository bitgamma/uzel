...
exports.DeviceManager.prototype.getDevicesByType = function(types, cb) {
  ...
  this._pairedDevices.find({ 'deviceInfo.type': { $in: types }}, function (err, foundDevices) {
    cb(foundDevices);
  });  
}

exports.DeviceManager.prototype.getDevicesByConnector = function(connector, cb) {
...
exports.DeviceManager.prototype.sendPacket = function(physicalDevice, commands) {
  //TODO: make timeout configurable per-packet
  var connector = this._connectorLoader.loadedModules[physicalDevice.connector];
  
  if ((commands.length == 1) || connector.allowMultipleCommandsInPacket) {
    connector.send(physicalDevice, this.createPacket(commands).encode(), 5000, processResponse.bind(this, commands));    
  } else {
    for (var i = 0, len = commands.length; i < len; i++) {
      connector.send(physicalDevice, this.createPacket([commands[i]]).encode(), 5000, processResponse.bind(this, commands));
    }    
  }
}
...
function handleUnpairedDevice(cb, device, err) {
  ...
  this._pairedDevices.remove({ id: device.id }, {}, function (err, numRemoved) {
...
function handleUpdatedDevice(device) {
  this._pairedDevices.update({ id: device.id }, device, {});
}
...
function initializeConnector(connectorName, connector) {
  ...
  connector.on('deviceUpdated', handleUpdatedDevice.bind(this));

  this.getDevicesByConnector(connectorName, function(pairedDevices) {
