...
function handlePairedDevice(cb, device, err) {
  handlePairingResponse(this._unpairedDevices, this._pairedDevices, cb, device, err)
}

function handleUnpairedDevice(cb, device, err) {
  handlePairingResponse(this._pairedDevices, this._unpairedDevices, cb, device, err)
}

function handlePairingResponse(srcArray, targetArray, cb, device, err) {
  if (err) {
    cb(device, err);
    return;
  }
  ...
  if (idx != -1) {
    this.srcArray.splice(idx);
  }
  
  this.targetArray.push(device);
  cb(device);
}
